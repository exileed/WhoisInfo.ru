...
// Whois-servers - https://www.iana.org/domains/root/db/
$whoisservers = array(
	"aarp" =>                     "whois.nic.aarp",
	...
	"рф" =>                       "whois.tcinet.ru", // .РФ домен | IDN
	"xn--p1ai" =>                 "whois.tcinet.ru", // .РФ домен | IDN
...
function LookupDomain($domain){
	...
	if(!$result) {
		return "Не было получено результатов из $whoisserver для домена: $domain!";
	}
	else {
	...
	return "Результаты проверки Whois-сервера ($whoisserver):\n\n" . $result;
}
...
function LookupIP($ip) {
	...
	foreach($results as $whoisserver=>$result) {
		$res .= "\n\n-------------\nРезультаты для " . $ip . " из " . $whoisserver . ":\n\n" . $result;
	}
	return $res;
}
...
if($domain) {
	...
	else echo("Неправильно введен домен или IPv4. Повторите запрос.<br><br>Если Вы ввели в поле домен с протоколом или ссылкой, пожалуйста -<br>удалите ненужную информацию и оставьте домен в виде: <b>example.ru</b><br><br>Если данная ошибка продолжает появляться - свяжитесь с тех.админом.");
	echo "<pre>\n" . $result . "\n</pre>\n";
}
?>
	  </div>
	   ...
	   <br><hr><br>
	   <p><a href=""><b>WhoisInfo.ru</b></a> - веб-инструмент для проверки занятости домена или получения информации о его владельце. Введите интересующий Вас домен в "поле" выше и получите информацию в течении одной секунды.</p>
		  <br>
		   <br>
